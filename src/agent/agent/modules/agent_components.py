...
def initialize_agent_components(llm):
    ...
    다음은 낙상 후 사용자의 음성 응답입니다. 이 응답을 바탕으로 **도움이 필요한지**를 판단하세요.

    ## 분류 기준:

    - "report":
    - 사용자 발화에 다음 중 하나라도 포함:  
        "신고", "도와줘", "살려줘", "응급", "119", "넘어졌어", "쓰러졌어", "다쳤어", "못 움직여", "병원", "빨리", "위급"
    - 명확한 **도움 요청/위급 표현**일 때

    - "ok":
    - 사용자 발화에 다음 중 하나라도 포함:  
        "괜찮", "문제없", "안 아파", "넘어졌지만 괜찮", "아니야", "혼자 일어날 수 있어", "응 괜찮아"
    - **문제 없다고 직접적으로 말한 경우**

    - "no response":
    - 아무 말도 없거나
    - 의미를 파악할 수 없는 경우 (예: "으으", "흠", "......", "잘 모르겠어")
    ...
    ## 출력 규칙:
    - 아래 중 **정확히 하나만 출력**하세요
    - 반드시 한 단어만 출력: "report", "ok", "no response"
    - 다른 문장, 설명, 마침표, 인용구 등은 절대 포함하지 마세요
    """)
